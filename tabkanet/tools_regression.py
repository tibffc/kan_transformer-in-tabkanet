...
from TabKANet.tabkanet.metrics import root_mean_squared_logarithmic_error
from sklearn.metrics import mean_squared_error, mean_absolute_error, r2_score

from TabKANet.tabkanet.dataset import TabularDataset
...
def train(model: torch.nn.Module, epochs: int, task: Literal['regression', 'classification'],
    ...
    for epoch in tqdm(range(epochs), desc='Epochs'):
        ...
        with torch.no_grad():
                ...
                val_mse, val_mae, val_rmse, val_corr =compute_regression_metrics(y_pred,y_true)

                if val_metric<best_rmse:
        ...
        with torch.no_grad():
                ...
                test_mse, test_mae, test_rmse, test_corr =compute_regression_metrics(y_pred,y_true)
        ...
        if custom_metric is not None:
            tqdm.write(f'Epoch: {epoch}, Train Loss: {loss.item():.4f}, Val Loss: {val_loss:.4f},Val MSE:{val_mse:.4f},Val MAE:{val_mae:.4f},Val RMSE:{val_rmse:.4f},Val R2:{val_corr:.4f},Val Metric: {val_metric:.4f}, Test Metric: {test_metric:.4f},Test MSE:{test_mse:.4f},Test MAE:{test_mae:.4f},Test RMSE:{test_rmse:.4f},Test R2:{test_corr:.4f}          ')
        else:
    ...
    print(f"FINISHED TRAINING, BEST VAL rmse:{best_rmse:.4f}")
