...
class TabularDataset(Dataset):
    def __init__(self,
        ...
        if categorical_features is not None:
            ...
            # 向量化转换：一次性将整个分类数据转为tensor
            cat_data = []
            for col in categorical_features:
                mapped = dataframe[col].map(self.vocabulary[col]).values.astype(np.int64)
                cat_data.append(mapped)
            self.categorical_data = torch.tensor(np.column_stack(cat_data), dtype=torch.long) if cat_data else None
        else:
            self.categorical_data = None

        # 连续特征：在初始化时完成类型转换，存储为tensor
        if continuous_features is not None:
            for column in continuous_features:
                if column not in dataframe.columns:
                    raise ValueError(f'{column} not found in dataframe')
            # 使用pandas向量化转换（比Python循环快100倍），无法转换的设为0
            cont_df = dataframe[continuous_features].apply(pd.to_numeric, errors='coerce').fillna(0)
            self.continuous_data = torch.tensor(cont_df.values, dtype=torch.float32)
        else:
            self.continuous_data = None
    ...
    def __getitem__(self, idx) -> Tuple[Optional[torch.Tensor], Optional[torch.Tensor], torch.Tensor]:
        ...
        if self.continuous_data is None:
            ...
            continuous_data = self.continuous_data[idx]
